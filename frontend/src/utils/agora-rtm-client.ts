...
function missingAgoraAppIdError() {
  const err: any = new Error('Missing REACT_APP_AGORA_APP_ID');
  err.reason = 'Missing REACT_APP_AGORA_APP_ID. Set frontend/.env.local before testing live collaboration.';
  return err;
}
...
export default class AgoraRTMClient {
  ...
  constructor () {
    ...
    this._client = APP_ID ? AgoraRTM.createInstance(APP_ID, { enableLogUpload: ENABLE_LOG, logFilter }) : null;
  }
  ...
  destroy (): void {
    ...
    this._currentChannel = null;
    this._currentChannelName = null;
    this._client && this._client.removeAllListeners();
  }
  ...
  async login (uid: string, token?: string) {
    if (!APP_ID) throw missingAgoraAppIdError();
    if (!this._client) {
